  ...
  if (!isAuthenticated) {
    return (
      <div className="h-screen bg-crust flex items-center justify-center p-4 font-sans">
        <div className="bg-mantle border border-surface0 p-8 rounded-2xl shadow-2xl w-full max-w-md text-center relative">
          ...
          <p className="text-subtext0 text-sm mb-8">{t('adminPrompt')}</p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              />
              {error && <p className="text-red text-xs mt-2 font-bold uppercase tracking-widest">{t('incorrectPassword')}</p>}
            </div>
            <button
              type="submit"
              disabled={isSubmitting}
            ...
            >
              {isSubmitting ? <Loader2 className="w-6 h-6 animate-spin mx-auto" /> : t('signIn')}
            </button>
          </form>
